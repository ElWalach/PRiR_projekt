...
void generateSymmetricPositiveDefiniteMatrixFast(vector<vector<double>>& A, 
                                                 vector<double>& b, 
...
Result sendToServer(const vector<vector<double>>& A, const vector<double>& b, 
    ...
    send(sock, buffer.data(), total * sizeof(double), 0);

    Result r{};
    recv(sock, &r, sizeof(r), 0);

    solution.resize(n);
    recv(sock, solution.data(), n * sizeof(double), 0);
...
void runTest(int n, double& t_seq, int& it_seq,
    ...
    generateSymmetricPositiveDefiniteMatrixFast(A, b, x_true, n);
